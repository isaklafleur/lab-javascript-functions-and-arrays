...
function uniquifyArray (array) {
  let uniqueArray = [];
  array.forEach(item => {
    uniqueArray.indexOf(item) === -1 ? uniqueArray.push(item) : null
  })
  return uniqueArray;
}
...
function howManyTimes (words, word) {
  let counter = 0;
  words.forEach(item => word === item ? counter++ : null);
  return counter;
}
...
function greatestProduct (matrix) {
  ...
  matrix.forEach((row, rowIndex, matrix) => {
    for (let columnIndex = 0; columnIndex < row.length; columnIndex++) {
     greatestProduct = Math.max(greatestProduct, calculateProductOnPosition(rowIndex, columnIndex, matrix)) 
    }
  });
  return greatestProduct;
}
  function calculateProductOnPosition(row, column, matrix) {
  const up = row < 1 ? 1 : matrix[row-1][column];
  const down = row === matrix.length-1 ? 1 : matrix[row+1][column];
  const left = column < 1 ? 1 : matrix[row][column-1];
  const right = column === matrix[row].length-1 ? 1 : matrix[row][column+1];
